...
struct PipelineKey
{
    ...
    bool operator==(const PipelineKey& b) const
    {
        ...
        && vertexDescriptor == b.vertexDescriptor
        ;
    }
    ...
    struct Hash
    {
        size_t operator()(const PipelineKey &key ) const
        {
            ...
            ^ (std::hash<uintptr_t>()((uintptr_t)key.vertexDescriptor) << 0)
                  ;
        }
    };
...
class MetalDynamicBuffer : public DynamicBuffer
{
public:
    ...
    virtual void WriteBytes(const void *data, size_t alignment, size_t length) override
    {
        // align position
...
//    virtual void Clear(Color4F color);
    ...
    virtual void UploadVertexData(size_t size, const Vertex *data) override;
...
static bool _Hlsl2Glsl_IncludeOpenFunc(bool isSystem, const char* fname, const char* parentfname, const char* parent, std::string& output, void* data)
{
    ...
    std::string headerPath = PathCombine(*rootDir, fname);
    if (!FileReadAllText(headerPath, output)) {
        return false;
    }
    return true;
}
...
public:
    ...
    virtual id <MTLRenderCommandEncoder>  Flush(MetalContext *context)
    {
        ...
        // set options
#if USE_POINT_SIZE
        [encoder setVertexBytes: &context->m_options
                         length: sizeof(context->m_options)
                        atIndex: 1];
#endif
    ...
    MetalShaderInfoPtr CompileMetal(const ShaderSource &source)
    {
        ...
        if (error != nil) {
            ...
            LogError( "Metal compilation failed: %s\n", error.description.UTF8String );
            return nullptr;
        }
...
//        std::cout << info->source << '\n';
    ...
    MTLRenderPipelineReflection *GetReflectionInfo()
    {
        ...
        psd.vertexDescriptor = _context->_vertexDescriptor;
...
//    auto _view = context->_view;
    ...
    psd.vertexDescriptor = context->_vertexDescriptor;

    psd.colorAttachments[0].pixelFormat = key.pixelFormat;
    psd.depthAttachmentPixelFormat = MTLPixelFormatInvalid;
    psd.stencilAttachmentPixelFormat = MTLPixelFormatInvalid;

    bool blendingEnabled = !(key.sourceFactor == BLEND_ONE && key.destFactor == BLEND_ZERO);
...
static MTLVertexDescriptor *CreateVertexDescriptor()
{
    ...
    vd.attributes[VertexAttribPosition].format = MTLVertexFormatFloat3;
    vd.attributes[VertexAttribPosition].offset = offsetof(Vertex, x);
    vd.attributes[VertexAttribPosition].bufferIndex = 0;

    vd.attributes[VertexAttribColor].format = MTLVertexFormatUChar4Normalized;
    vd.attributes[VertexAttribColor].offset = offsetof(Vertex, Diffuse);
    vd.attributes[VertexAttribColor].bufferIndex = 0;

    vd.attributes[VertexAttribTexCoord0].format = MTLVertexFormatFloat2;
    vd.attributes[VertexAttribTexCoord0].offset = offsetof(Vertex, tu);
    vd.attributes[VertexAttribTexCoord0].bufferIndex = 0;

    vd.attributes[VertexAttribTexCoord1].format = MTLVertexFormatFloat2;
    vd.attributes[VertexAttribTexCoord1].offset = offsetof(Vertex, rad);
    vd.attributes[VertexAttribTexCoord1].bufferIndex = 0;
...
static TexturePtr CreateWhiteTexture(Context *context)
{
    ...
    for (int i=0; i < size * size; i++)
        data[i] = 0xFFFFFFFF;
    auto texture = context->CreateTexture("white", size, size, PixelFormat::RGBA8Unorm, data);
    delete[] data;
    return texture;
}
...
{
    SetRenderTarget(nullptr, nullptr, LoadAction::Load);
    SetBlendDisable();
...
id <MTLRenderCommandEncoder> MetalContext::GetEncoder()
{
    ...
    if (!m_renderTarget)
    {
        ...
        auto drawable = _view.currentDrawable;
        auto texture = drawable.texture;
        m_renderTargetSize   = Size2D( (int) texture.width,  (int)texture.height);
...
void MetalContext:: Present()
{
    ...
    uintptr_t async_id = (uintptr_t) (__bridge void *)_commandBuffer;
    
    PROFILE_DECLARE_SECTION(_cbuffer_commit_section, "CommandBuffer::Commit", "gpu")
    TProfiler::AddAsyncBegin(_cbuffer_commit_section, async_id);
...
//void MetalContext::SetRenderTarget(TexturePtr texture)
//{
//    SetRenderTarget(texture, nullptr);
//}
//
//void MetalContext::SetRenderTargetDiscard(TexturePtr texture, const char *passName)
//{
//    SetRenderTarget(texture, passName);
//    _loadAction = MTLLoadActionDontCare;
//}
...
void MetalContext::UploadIndexData(int indexCount, const IndexType *indexData)
{
    _indexBuffer.Write(indexData, indexCount);
}

void MetalContext::UploadVertexData(size_t vertexCount, const Vertex *vertexData)
{
    _vertexBuffer.Write(vertexData, vertexCount);
    
    // set vertex descriptor
    _key.vertexDescriptor = _vertexDescriptor;
