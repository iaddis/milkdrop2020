...
public:
    ...
    virtual bool Flush(GLContext *context)
    {
        ...
        if (m_constant_global_yscale)
        {
           ...
           m_constant_global_yscale->SetFloat(yscale);
        }

        if (m_constant_global_pointsize)
        {
            m_constant_global_pointsize->SetFloat(context->GetPointSize());
        }
...
//        m_verbose= true;
        ...
        if (m_verbose)
            LogPrint("ConvertHLSLToGLSL(%s, %s):\n%s\n", source.profile.c_str(),
...
GLContext::GLContext()
{
    ...
    LogPrint("GL_RENDERER: %s\n", glGetString(GL_RENDERER));
    LogPrint("GL_SHADING_LANGUAGE_VERSION: %s\n", glGetString(GL_SHADING_LANGUAGE_VERSION));
...
//    assert(m_displaySize.width > 0 && m_displaySize.height > 0);
    ...
    glVertexAttribPointer(VertexAttribPosition, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void *)offsetof(Vertex, x));
    ...
    glVertexAttribPointer(VertexAttribTexCoord0, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void *)offsetof(Vertex, tu));
    glEnableVertexAttribArray(VertexAttribTexCoord1);
    glVertexAttribPointer(VertexAttribTexCoord1, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void *)offsetof(Vertex, rad));
    glBindVertexArray(0);
...
TexturePtr GLContext::CreateRenderTarget(const char *name, int width, int height, PixelFormat format)
{
    ...
    if (error)
    {
        LogError("Could not create texture with format: %s\n", PixelFormatToString(format));
...
void GLContext::UploadIndexData(int indexCount, const IndexType *indexData)
{
    // upload index data
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_ib);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, indexCount * sizeof(indexData[0]), indexData, GL_DYNAMIC_DRAW);
    
}

void GLContext::UploadVertexData(size_t size, const Vertex *data)
{
    glBindBuffer(GL_ARRAY_BUFFER, m_vb);
    glBufferData(GL_ARRAY_BUFFER, size * sizeof(data[0]), data, GL_DYNAMIC_DRAW);
